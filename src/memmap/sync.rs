use skl::{either::Either, u5, Arena, MmapOptions, OpenOptions, Options as MapOptions, SkipMap};
...
pub enum Error<S: Snapshot> {
  ...
  #[cfg_attr(feature = "std", error("snapshot has bad magic text"))]
  BadMagicText,
  /// Cannot open snapshot because the external magic doesn't match.
  #[cfg_attr(feature = "std", error("cannot open snapshot because the external magic doesn't match. expected {expected}, found {found}"))]
  BadExternalMagic {
...
#[cfg(not(feature = "std"))]
impl<M: BackedSnapshot> core::fmt::Display for Error<S> {
  #[inline]
