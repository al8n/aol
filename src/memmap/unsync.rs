  ...
  fn append_batch<S: Snapshot, C: Checksumer>(
    ...
    match self {
      ...
      Memmap::MapMut { mmap, file } => {
        ...
        if len + total_encoded_size > opts.capacity {
          let (new_len, this) =
            Self::rewrite_in::<S, C>(mmap, file, filename, rewrite_filename, snapshot, opts, len)?;

          // if after rewrite, the entry is still too large, return error
          if new_len + total_encoded_size > opts.capacity {
            return Err(Error::entry_corrupted(
              total_encoded_size as u32,
              (opts.capacity - len) as u32,
            ));
          }
  ...
  fn append<S: Snapshot, C: Checksumer>(
    ...
    match self {
      ...
      Memmap::MapMut { mmap, file } => {
        ...
        if len + encoded_len > opts.capacity {
          let (new_len, this) =
            Self::rewrite_in::<S, C>(mmap, file, filename, rewrite_filename, snapshot, opts, len)?;

          // if after rewrite, the entry is still too large, return error
          if new_len + encoded_len > opts.capacity {
            return Err(Error::entry_corrupted(
              encoded_len as u32,
              (opts.capacity - len) as u32,
            ));
...
impl<S: Snapshot, C: Checksumer> AppendLog<S, C> {
  ...
  pub fn append(&mut self, ent: Entry<S::Data>) -> Result<(), Error<S>> {
    ...
    self.len = self.file.as_mut().unwrap().append::<S, C>(
      &self.filename,
      &self.rewrite_filename,
      &mut self.snapshot,
      &self.opts,
  ...
  pub fn append_batch(&mut self, entries: Vec<Entry<S::Data>>) -> Result<(), Error<S>> {
    ...
    self.len = self.file.as_mut().unwrap().append_batch::<S, C>(
      &self.filename,
      &self.rewrite_filename,
      &mut self.snapshot,
      &self.opts,
